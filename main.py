...
def save_to_excel(df, file_path):
    ...
    worksheet = workbook.create_sheet(title='Analysis Results')

    # Write the headers in row 1
    headers = ['url_ID', 'URL', 'Positive Score', 'Negative Score', 'Polarity Score', 'Subjectivity Score',
               'Average Sentence Length', 'Percentage of Complex Words', 'Fog Index', 'Average Words Per Sentence']
    
    for col_index, header in enumerate(headers, start=1):  # Start writing from column G (7th column)
        worksheet.cell(row=1, column=col_index, value=header)

    # Write the data in rows starting from row 2
    for row_index, row in df.iterrows():
        for col_index, value in enumerate(row, start=1):  # Start writing from column G (7th column)
            worksheet.cell(row=row_index + 2, column=col_index, value=value)

    workbook.save(file_path)

# Example usage:
file_path = 'url_file.xlsx'  # Use the same file for input and output
positive_keywords_file = 'positive_keywords_file.txt'
negative_keywords_file = 'negative_keywords_file.txt'

df_urls, positive_keywords, negative_keywords = read_input_data(file_path, positive_keywords_file, negative_keywords_file)
if df_urls is not None and positive_keywords is not None and negative_keywords is not None:
    ...
    # Save the results to the same Excel file in columns G to N
    save_to_excel(results_df, file_path)
